...
# 加载资金账号->二级营业部映射
with open('C:/账号梳理/低延时_dept2_mapping.json', 'r', encoding='utf-8') as f:
    mapping = json.load(f)
...
while True:
    cmd = 'lark-cli base +record-list --base-token {} --table-id "{}" --limit {} --offset {} --field-id 资金账号 --field-id 营业部 --field-id ID'.format(
...
dept_idx = all_fields.index("营业部") if "营业部" in all_fields else -1

print(f"资金账号索引: {fund_idx}, 营业部索引: {dept_idx}")
...
for idx, record in enumerate(all_records):
    if fund_idx < 0 or dept_idx < 0 or idx >= len(all_record_ids):
        continue

    fund_id = str(record[fund_idx]) if fund_idx < len(record) and record[fund_idx] else ""
    current_dept = str(record[dept_idx]) if dept_idx < len(record) and record[dept_idx] else ""
    ...
    if fund_id in mapping:
        new_dept = mapping[fund_id]
        if current_dept != new_dept:
            # 更新
            update_cmd = 'lark-cli base +record-upsert --base-token {} --table-id "{}" --record-id {} --json "{{\\"营业部\\":\\"{}\\"}}"'.format(
                BASE_TOKEN, TABLE_ID, record_id, new_dept
            )
            ...
            if result.returncode == 0:
                print(f"更新 record_id={record_id}, 资金账号={fund_id}, 营业部: {current_dept} -> {new_dept}")
                updated += 1
            else:
